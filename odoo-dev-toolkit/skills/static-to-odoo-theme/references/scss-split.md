...
**MANDATORY: Add `<field name="sequence">` to every `theme.ir.asset` record.** Without explicit sequence fields, Odoo can reorder assets unpredictably within the same bundle — `responsive.scss` may load before `theme.scss`, causing all media queries to be silently ignored (same specificity, earlier in cascade = loses). Use: theme=100, header=110, footer=120, responsive=200.

## Step 9: Color variable safety check (CRITICAL)

After splitting, run this sweep to catch `var(--o-color-X)` usage that will break when users change Theme panel colors:

```bash
# Find var(--o-color-X) in component SCSS
grep -rn 'var(--o-color-' static/src/scss/theme.scss \
    static/src/scss/header.scss \
    static/src/scss/footer.scss

# Find var(--o-color-X) in inline XML styles
grep -rn 'var(--o-color-' views/
```

**Every match is a bug** (except in `primary_variables.scss` palette definitions).

`var(--o-color-1)` through `var(--o-color-5)` are CSS custom properties generated by Odoo's `o_cc` classes. They are NOT fixed — the user can rearrange colors via the Theme panel, which reassigns which hex maps to which `--o-color-N`. Components using these for contrast-critical styling (white card on dark section, dark text on light card) will silently break.

**Fix:** Define fixed SCSS variables at the top of each file and use those instead:
...
// CORRECT
.my-card { background: $brand-white; color: $brand-dark-text; }

// WRONG — user can break this via Theme panel
.my-card { background: var(--o-color-1); color: var(--o-color-2); }
```

For inline styles in XML snippets, use fixed hex values: `style="color: #2E86DE;"` not `style="color: var(--o-color-4);"`.

See `odoo-theme/references/theme-scss-architecture.md` → "CRITICAL: var(--o-color-X) vs Fixed SCSS Variables" for the full explanation.
