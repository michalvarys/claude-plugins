...
## CRITICAL: var(--o-color-X) vs Fixed SCSS Variables (hard-earned)

**Problem**: `var(--o-color-1)` through `var(--o-color-5)` are CSS custom properties generated by Odoo's `o_cc` classes on sections. They are NOT fixed — the user can rearrange colors via the Theme panel (Website → Customize → Theme), which reassigns which hex value each `--o-color-N` maps to. For example, `--o-color-1` might be `#FFFFFF` in the palette definition but become `rgba(10, 22, 40, 0.95)` if the user swaps colors in the Theme panel.

**Consequence**: If you use `var(--o-color-1)` for a white card background and the user changes colors in Theme panel, the card background becomes dark — with dark text on dark background (zero contrast).

**Rule**: NEVER use `var(--o-color-X)` in `theme.scss`, `header.scss`, or `footer.scss` for component-level styling where contrast matters. Instead, define fixed SCSS variables for your brand colors and use those:

```scss
// Fixed brand colors — independent of Theme panel changes
$vx-white:            #FFFFFF;
$vx-dark-text:        #333333;
$vx-light-bg:         #F4F6F8;
$vx-accent-hex:       #2E86DE;
$vx-primary-hex:      #0A1628;

// CORRECT — guaranteed contrast
.my_card {
    background: $vx-white;
    color: $vx-dark-text;
}

// WRONG — user can break contrast via Theme panel
.my_card {
...
**Where var(--o-color-X) IS safe**: `primary_variables.scss` palette definitions and `o-cc*` override mappings. These define the system — they don't consume it.

**Where var(--o-color-X) is DANGEROUS**: Any component that needs guaranteed foreground/background contrast — cards, buttons, headings, form inputs, icon circles, badges. If the user reorders palette colors, these break silently.

**The same applies to inline styles in XML**: `style="color: var(--o-color-4);"` in snippet templates is equally fragile. Use fixed hex values: `style="color: #2E86DE;"`.

## Offcanvas Drawer on Mobile (hard-earned)

**Problem 1: Links don't close the drawer** — When a navigation link inside Bootstrap offcanvas points to an anchor on the same page (e.g. `/contact#quote`), clicking it does NOT close the offcanvas because no page reload occurs.

**Fix**: Add `data-bs-dismiss="offcanvas"` to ALL `<a>` elements inside the offcanvas body. This closes the drawer on click regardless of whether the page reloads:

```xml
<a class="nav-link" data-bs-dismiss="offcanvas"
   t-att-href="menu.url" t-out="menu.name"/>
```

**Problem 2: Content mixes with drawer (z-index/stacking context)** — Odoo's header (`header#top`) or `.o_header_affixed` may have `transform` applied (for sticky header animation), which creates a new stacking context. This breaks `position: fixed` on the offcanvas inside, making it render inline with the page content instead of overlaying it.

**Fix**: Force `transform: none` on the header elements:
...
## Hidden Form Fields (.s_website_form_dnone)

Odoo's `s_website_form_dnone` class should hide fields like `email_to`, but custom theme SCSS can inadvertently override it by applying `display: block` or `display: flex` to form elements with higher specificity. Always add an explicit rule:
